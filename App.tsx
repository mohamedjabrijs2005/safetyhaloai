...
const App: React.FC = () => {
  ...
  const [confidenceThreshold, setConfidenceThreshold] = useState<number>(() => {
    const saved = localStorage.getItem(SETTINGS_KEY);
    return saved ? JSON.parse(saved).confidenceThreshold : 0.70;
  });
  ...
  const runAnalysis = useCallback(async (currentContext: RoomContext) => {
    // Threshold Check
    if (currentContext.ml_confidence < confidenceThreshold) {
      setSafetyReport({
        status: SafetyStatus.SAFE,
        summary: `Analysis bypassed: Context confidence (${(currentContext.ml_confidence * 100).toFixed(0)}%) is below the security gate threshold (${(confidenceThreshold * 100).toFixed(0)}%).`,
        actions_for_user: ["Monitor raw metrics manually", "Wait for higher confidence state"],
        actions_for_warden: ["No action required - below threshold."]
      });
      return;
    }
  ...
  }, [playAlertSound, confidenceThreshold]);

  const triggerScenario = (key: keyof typeof SCENARIOS) => {
    const scenario = SCENARIOS[key];
    const generatedConfidence = 0.55 + Math.random() * 0.45; // Simulated confidence variance
    ...
    <div className="min-h-screen flex flex-col lg:flex-row bg-slate-50 text-slate-900 pb-20 lg:pb-0">
      <aside className="w-64 bg-white border-r border-slate-200 hidden lg:flex flex-col sticky top-0 h-screen">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <div className="pt-4 pb-2 px-3 space-y-4">
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Configuration Settings</p>
            ...
            <div className="bg-slate-50 p-4 rounded-xl space-y-3 border border-slate-100 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2 text-slate-600">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-tight">Confidence Gate</span>
                </div>
                <div className="flex items-center space-x-1">
                  <span className="text-[11px] font-black text-white bg-indigo-600 px-2 py-0.5 rounded-full shadow-sm">{(confidenceThreshold * 100).toFixed(0)}%</span>
                </div>
              </div>
              <input 
                type="range" 
              ...
              <p className="text-[9px] text-slate-400 leading-tight border-t border-slate-200 pt-2 italic">
                AI reasoning is only triggered when state confidence exceeds this threshold.
              </p>
            </div>
          </div>
        </nav>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="sticky top-0 z-30 bg-slate-50 bg-opacity-80 backdrop-blur-md border-b border-slate-200 px-4 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
             <div className="bg-white border border-slate-200 rounded-full px-3 lg:px-4 py-1.5 flex items-center space-x-3 shadow-sm text-[10px] lg:text-sm">
                <Target size={16} className="text-indigo-500" />
                <div className="flex flex-col">
                  <span className="text-[9px] font-bold text-slate-400 uppercase leading-none">Security Gate</span>
                  <span className="font-black text-indigo-600">{(confidenceThreshold * 100).toFixed(0)}% Confidence</span>
                </div>
             </div>
          </div>
        </header>
          ...
          {activeTab === 'dashboard' ? (
            <>
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="xl:col-span-2 space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg lg:text-xl font-bold flex items-center space-x-2 text-slate-800">
                ...
                <div className="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-slate-200 flex flex-col min-h-[400px]">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                      ...
                      <h3 className="text-lg lg:text-xl font-bold">AI Context Report</h3>
                    </div>
                    {isAnalyzing && (
                  ...
                  {safetyReport ? (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                      <div>
                        <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-2 tracking-widest flex items-center justify-between">
                          <span>Analysis Outcome</span>
                          {mlConfidence < confidenceThreshold && (
                            <span className="text-amber-500 bg-amber-50 px-2 py-0.5 rounded text-[8px] border border-amber-100 uppercase">Confidence Gate Active</span>
                          )}
                        </h4>
                        <p className={`text-base lg:text-lg leading-relaxed font-medium italic ${mlConfidence < confidenceThreshold ? 'text-slate-500 opacity-80' : 'text-slate-700'}`}>
                        ...
                        <div>
                          ...
                          <ul className="space-y-2">
                            {safetyReport.actions_for_warden.map((action, i) => (
                              <li key={i} className="flex items-start space-x-2 text-xs lg:text-sm text-slate-600">
                                <span className="mt-1 text-slate-400 font-bold">•</span>
                                <span>{action}</span>
                              </li>
                            ))}
                          </ul>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center opacity-40">
                      ...
                      <p className="text-sm">Waiting for context trigger...</p>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-200">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <div className="flex items-center space-x-3">
                    ...
                    <h3 className="text-lg lg:text-xl font-bold text-slate-800">Historical Event Logs</h3>
                  </div>
                  <div className="flex items-center space-x-3">
                    <button 
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-slate-50/50 text-slate-400 text-[10px] font-bold uppercase tracking-widest border-b border-slate-100">
                        ...
                        <th className="px-6 py-4 hidden sm:table-cell">Full Data Snapshot</th>
                      </tr>
                    </thead>
                        ...
                        <tr>
                          <td colSpan={4} className="px-6 py-16 text-center text-slate-400">
                            <div className="flex flex-col items-center justify-center opacity-50">
                              ...
                              <p className="text-sm font-medium tracking-wide">No activity recorded yet.</p>
                              <p className="text-[10px] mt-1 italic">Use the Context Control scenarios to generate data.</p>
                            </div>
                          </td>
                        </tr>
                      )}
            ...
            <div className="max-w-4xl mx-auto space-y-12 pb-20">
              <section className="space-y-4">
                ...
                <h2 className="text-3xl lg:text-4xl font-black tracking-tight">System Architecture</h2>
                <p className="text-slate-600 text-base lg:text-lg leading-relaxed">
                  SafetyHalo AI utilizes a tiered context-classification system. Raw sensor streams (motion, sound, gas, temp) 
                  are first processed by an edge ML model to determine a "Context State". High-confidence states are then sent 
                  to <strong>Gemini 3 Flash</strong> for natural language reasoning and proactive protocol generation.
                </p>
              </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-white p-6 lg:p-8 rounded-3xl border border-slate-200 space-y-4 shadow-sm">
                  ...
                  <h3 className="text-xl font-bold">Data Fusion Tier</h3>
                  <p className="text-slate-500 text-sm leading-relaxed">
                    Sensors are polled at 10Hz. We use temporal correlation to identify events like "Silence after Motion" 
                    or "Rapid Temp Rise with Gas Detection" to minimize false alarms.
                  </p>
                </div>
                <div className="bg-white p-6 lg:p-8 rounded-3xl border border-slate-200 space-y-4 shadow-sm">
                  <div className="w-10 h-10 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600">
                  ...
                  <h3 className="text-xl font-bold">Confidence Gating</h3>
                  <p className="text-slate-500 text-sm leading-relaxed">
                    The Security Gate ensures expensive AI reasoning only occurs when the edge classifier has a high 
                    certainty about the current environment, optimizing both safety and system resources.
                  </p>
                </div>
              </div>

              <section className="space-y-6">
                <h3 className="text-2xl font-black text-slate-800">Safety Status Hierarchy</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
                  <div className="p-6 bg-white border border-slate-100 rounded-3xl space-y-3 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <p className="text-[11px] text-slate-500 leading-relaxed">Ambient state. Environment is within expected limits and activity patterns are consistent with scheduled occupancy.</p>
                  </div>
                  <div className="p-6 bg-white border border-slate-100 rounded-3xl space-y-3 shadow-sm hover:shadow-md transition-shadow">
                    <ShieldAlert className="text-amber-500" size={32} />
                    <h4 className="font-bold text-amber-700">WARNING</h4>
                    <p className="text-[11px] text-slate-500 leading-relaxed">Anomalous pattern detected. AI suggests proactive checks due to deviations in noise levels or motion sequences.</p>
                  </div>
                  <div className="p-6 bg-white border border-slate-100 rounded-3xl space-y-3 shadow-sm hover:shadow-md transition-shadow">
                    <ShieldX className="text-red-500" size={32} />
                    <h4 className="font-bold text-red-700">DANGER</h4>
                    <p className="text-[11px] text-slate-500 leading-relaxed">Critical hazard event. High-certainty detection of environmental danger or physical distress. Emergency protocol active.</p>
                  </div>
                </div>
              </section>
              ...
              <div className="bg-slate-900 rounded-3xl p-8 lg:p-12 text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-2 text-center md:text-left">
                  ...
                  <p className="text-slate-400 text-sm">All sensor data is anonymized before processing. We use Gemini 3's advanced safety filters.</p>
                </div>
                <a 
